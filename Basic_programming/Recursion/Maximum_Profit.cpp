...
int helper(int *budget, int n, int total)
{
  if (n == 0)
  {
    return total;
  ...
  if (budget[0] * n > total)
  {
    int newTotal = budget[0] * n;
    return helper(budget + 1, n - 1, newTotal);
  }
  return helper(budget + 1, n - 1, total);
}
